...
// const model = "gemini-2.0-flash" // Model name for the AI client
...
// 	RefineItinerary(ctx context.Context, sessionID string, userID uuid.UUID, profileID uuid.UUID, cityName string, refinementRequest string, existingItinerary types.AIItineraryResponse) (*types.AIItineraryResponse, error)
...
// // Session Management:
// // The activeChatSessions map will grow indefinitely. Add a cleanup mechanism (e.g., a TTL or explicit session removal) to prevent memory leaks.

// // Error Handling:
// // If the session isn’t found in RefineItinerary, it returns an error. You could alternatively recreate the session with the existing itinerary as context, but this might complicate the flow.

// // Concurrency:
// // The mutex ensures thread safety, but keep lock duration minimal by performing SendMessage outside the lock.
...
// 	defer wg.Done()
// 	startTime := time.Now()
// 	sessionID := fmt.Sprintf("%s-%s-%s", userID.String(), profileID.String(), cityName)
// 	sessionsMu.Lock()
// 	_, ok := sessions[sessionID]
// 	if !ok {
// 		// Create a new chat session
// 		chats := &genai.Chats{} // Assume aiClient has apiClient
// 		chat, err := chats.Create(ctx, model, config, nil)
// 		if err != nil {
// 			sessionsMu.Unlock()
// 			resultCh <- types.GenAIResponse{Err: fmt.Errorf("failed to create chat session: %w", err)}
// 			return
// 		}
...
// 	// Extract POI names from the itinerary
// 	poiNames := make([]string, 0, len(itinerary.AIItineraryResponse.PointsOfInterest))
// 	for _, poi := range itinerary.AIItineraryResponse.PointsOfInterest {
// 		poiNames = append(poiNames, poi.Name)
// 	}

// 	// Fetch sorted POIs using PostGIS
// 	sortedPOIs, err := l.poiRepo.GetPOIsByNamesAndCitySortedByDistance(ctx, poiNames, cityID, userLocation)
// 	if err != nil {
// 		l.logger.ErrorContext(ctx, "Failed to retrieve sorted POIs", slog.Any("error", err))
// 		return nil, fmt.Errorf("failed to retrieve sorted POIs: %w", err)
// 	}

// 	// Update the itinerary with sorted POIs
...
// func (l *LlmInteractiontServiceImpl) RefineItinerary(ctx context.Context, sessionID string, userID uuid.UUID, profileID uuid.UUID, cityName string, refinementRequest string, existingItinerary types.AIItineraryResponse) (*types.AIItineraryResponse, error) {
// 	// Retrieve the chat session
// 	sessionsMu.Lock()
// 	chat, ok := sessions[sessionID]
// 	sessionsMu.Unlock()
// 	if !ok {
// 		return nil, fmt.Errorf("chat session not found for sessionID: %s", sessionID)
// 	}

// 	// Construct the refinement prompt
// 	prompt := fmt.Sprintf("Please refine the current itinerary based on this request: %s", refinementRequest)
// 	// Optionally include a summary of the existing itinerary if needed, though the chat history already contains it
// 	// prompt += fmt.Sprintf("\nCurrent itinerary: %s with POIs: %v", existingItinerary.ItineraryName, existingItinerary.PointsOfInterest)

// 	// Send the refinement request
// 	response, err := chat.SendMessage(ctx, genai.Part{Text: prompt})
// 	if err != nil {
// 		return nil, fmt.Errorf("failed to send refinement message: %w", err)
// 	}
...
// 	}
// 	if txt == "" {
// 		return nil, fmt.Errorf("no valid refinement content from AI")
// 	}

// 	// Parse the JSON response
// 	jsonStr := strings.TrimPrefix(strings.TrimSuffix(strings.TrimSpace(txt), "```"), "```json")
// 	var refinedItinerary struct {
// 		ItineraryName      string            `json:"itinerary_name"`
// 		OverallDescription string            `json:"overall_description"`
// 		PointsOfInterest   []types.POIDetail `json:"points_of_interest"`
// 	}
// 	if err := json.Unmarshal([]byte(jsonStr), &refinedItinerary); err != nil {
// 		return nil, fmt.Errorf("failed to parse refined itinerary JSON: %w", err)
// 	}

// 	// Return the refined itinerary
// 	return &types.AIItineraryResponse{
// 		ItineraryName:      refinedItinerary.ItineraryName,
// 		OverallDescription: refinedItinerary.OverallDescription,
// 		PointsOfInterest:   refinedItinerary.PointsOfInterest,
// 	}, nil
// }
