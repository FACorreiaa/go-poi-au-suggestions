...
func (m *MockUserRepo) GetUserEnhancedInterests(ctx context.Context, userID uuid.UUID) ([]types.EnhancedInterest, error) {
	args := m.Called(ctx, userID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).([]types.EnhancedInterest), args.Error(1)
}

func (m *MockUserRepo) GetUserPreferenceProfiles(ctx context.Context, userID uuid.UUID) ([]types.UserPreferenceProfile, error) {
	args := m.Called(ctx, userID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
